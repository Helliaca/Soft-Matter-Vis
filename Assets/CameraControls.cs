	...
	void LateUpdate () {
		...
		if (target && isActivated) { 
			...
			{	
				...
				if(Input.GetKey(KeyCode.KeypadMinus)) distance = ZoomLimit(distance + scrollSpeed*0.1f, zoomMin, zoomMax);
				if(Input.GetKey(KeyCode.KeypadPlus)) distance = ZoomLimit(distance - scrollSpeed*0.1f, zoomMin, zoomMax);
